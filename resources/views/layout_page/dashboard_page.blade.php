...
document.addEventListener('click', function (event) {
    if (event.target.matches('.btn-warning')) {
        ...
        let bookTitle = event.target.getAttribute('data-title'); // Get the book title

        // Set values in modal
        document.getElementById('book_id_input').value = bookId; // Hidden input for ID
        document.getElementById('book_title').innerText = bookTitle; // Display title in modal
    }
});
...
for (let i = 0; i < response.borrowed.borrowed_books.length; i++) {
    ...
    let isBorrowed = response.borrowed.borrowed_books[i].status; // Assuming 'status' is a boolean
